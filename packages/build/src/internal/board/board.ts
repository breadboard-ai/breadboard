 ...
 * @return A {@link OldBoardDefinition} which can be serialized for execution or
...
export type BoardDefinition<
  I extends Record<string, JsonSerializable | undefined> = Record<
    string,
    JsonSerializable | undefined
  >,
  O extends Record<string, JsonSerializable | undefined> = Record<
    string,
    JsonSerializable | undefined
  >,
> = BoardInstantiateFunction<I, O> & SerializableBoard;
...
type SimplifyBoardInitOutputs<T extends BoardInit["outputs"]> =
  T extends OutputNode<infer X>
    ...
    : T extends OutputNode[]
      ? /* TOOD(aomarks) This recursion is incorrect here. For outputs, we can't maintain the outer-union. We need to collapse e.g. {foo} | {bar} to {foo? bar?}. */
...
export function inputNode<T extends Record<string, GenericSpecialInput>>(
  ...
  // TODO(aomarks) Implement! Also handle in serialize.
