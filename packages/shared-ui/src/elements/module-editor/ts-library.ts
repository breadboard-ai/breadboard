...
function getTypeScriptMod(mod: string) {
  switch (mod) {
    case "/lib.d.ts":
...
export function getMappedMinimalModList() {
  return minimalModList.map((mod) =>
    getTypeScriptMod(mod).then<[string, string]>((content) => [
      mod,
      content.default,
    ])
  );
...
export function getMappedQuickJsModList() {
  return quickJSModList.map((mod) =>
    getTypeScriptMod(mod).then<[string, string]>((content) => [
      mod,
      content.default,
    ])
...
export function getMappedFullModList() {
  return fullModList.map((mod) =>
    getTypeScriptMod(mod).then<[string, string]>((content) => [
      mod,
      content.default,
    ])
