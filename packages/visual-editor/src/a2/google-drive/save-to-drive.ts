/**
 * @fileoverview Save content to Google Drive (Docs, Slides, Sheets).
 */
...
async function saveToDrive(
  { context, mimeType, title, graphId, fileId }: SaveToDriveInputs,
  caps: Capabilities,
  moduleArgs: A2ModuleArgs
): Promise<Outcome<SaveToDriveOutputs>> {
  graphId ??= "";
...
async function getCollector(
  ...
  let id;
  if (!fileId) {
    const fileKey = `${getTypeKey(mimeType)}${graphId}`;
