...
json
  ...
  .wire("<-PINECONE_INDEX", core.secrets({ keys: ["PINECONE_INDEX"] }))
  .wire(
    "<-PINECONE_PROJECT_ID",
    core.secrets({ keys: ["PINECONE_PROJECT_ID"] })
  )
  .wire(
    "<-PINECONE_ENVIRONMENT",
    core.secrets({ keys: ["PINECONE_ENVIRONMENT"] })
  )
  .wire("<-PINECONE_API_KEY", core.secrets({ keys: ["PINECONE_API_KEY"] }))
  .wire(
    "result->config",
    board.output({
      $id: "result",
