...
const Utils = {
  Logging: {
    getLogger: () => console,
    Formatter: {
      verbose: (msg: unknown, ..._args: unknown[]) => msg,
      warning: (msg: unknown, ..._args: unknown[]) => msg,
      error: (msg: unknown, ..._args: unknown[]) => msg,
      group: (msg: unknown, ..._args: unknown[]) => msg,
    },
  },
};
...
class CoordinationRegistry {
  ...
  private inProgressActions = new Set<{
    id: number;
    name: string;
    mode: ActionMode;
  ...
  private nextInProgressId = 0;

  // Registry of all actions with their modes
  private registeredActions = new Map<string, ActionMetadata>();

  // Current execution stack (trigger and action names) - for debugging/logging
  private callStack: string[] = [];

  // Logger instance
  private logger = Utils.Logging.getLogger();
   ...
   */
  private exitTrigger(name: string): void {
