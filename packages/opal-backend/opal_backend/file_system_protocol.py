...
"""
FileSystem protocol — the contract that agent file system implementations
must satisfy.

Defines the ``FileSystem`` protocol and its supporting types. Both the
in-memory ``AgentFileSystem`` (for browser-based Opal) and the disk-backed
``DiskFileSystem`` (for bees) satisfy this protocol.

Consumers should type their file system parameters as ``FileSystem``, not
...
class FileSystemSnapshot:
    ...
    (the in-memory implementation). For disk-backed implementations
    this is a point-in-time capture of what's on disk.
...
def file_descriptor_to_part(file: FileDescriptor) -> dict[str, Any]:
    ...
    Extracted from ``AgentFileSystem._file_to_part`` so it can be
    shared across implementations without reaching into private methods.
    """
...
class FileSystem(Protocol):
    ...
    Implementations:
    - ``AgentFileSystem`` — in-memory, ``/mnt/``-prefixed (browser Opal)
    - ``DiskFileSystem`` — disk-backed, relative paths (bees)
    ...
    def write(self, name: str, data: str) -> str:
        ...
        HTML files may get ``inlineData`` type in some implementations.
        """
        ...
