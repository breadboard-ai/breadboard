...
| `sheet_manager.py`       | `sheet-manager.ts`, `memory-sheet-getter.ts`     | Memory sheet ops via DriveOperationsClient. TODO: add per-graph caching (TS uses `GraphCache`). |
...
| `memory.py`   | `functions/memory.ts`                     | 5 memory functions (create/read/update/delete sheet, get metadata) |
