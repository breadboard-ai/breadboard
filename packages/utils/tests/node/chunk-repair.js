...
const repair = async (incoming, repaired) => {
    const log = [];
    ...
    await incomingStream.pipeThrough(chunkRepairTransform()).pipeTo(new WritableStream({
        write(chunk) {
            log.push(chunk);
        },
    }));
...
describe("Chunk Repair Transform", async () => {
    ...
    test("chunks broken at boundary", async () => {
        await repair(['data: ["input"', ',{"node":{}}]\n', "\n"], ['data: ["input",{"node":{}}]\n\n']);
    });
});
//# sourceMappingURL=chunk-repair.js.map