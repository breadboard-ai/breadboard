...
import { InMemoryRunStore } from "../../src/run/in-memory-run-store.js";
...
it("InMemoryRunStore stores run events", async () => {
...
it("InMemoryRunStore replaces inlineData with storedData", async () => {
  const store = new InMemoryRunStore();
  await store.start("store");
...
it("InMemoryRunStore replaces storedData with inlineData when writing", async function () {
