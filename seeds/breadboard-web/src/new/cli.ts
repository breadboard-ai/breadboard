...
// This is a copy of graph-playground/src/index.ts, hacked up so we can test it
// with the runner being extracted out of the new syntax.
...
async function main(args: string[], use_input_handler = false) {
  ...
  probe.addEventListener("done", (event: Event) => {
    const { detail } = event as CustomEvent<{
      last: { descriptor: NodeDescriptor } | undefined;
      incompleteNextNodes: {
    ...
    console.log(detail);
    if (!detail.last) {
      note("queue was empty", "done");
    } else if (detail.last.descriptor.type !== "output") {
      note(
        `Last invoked node ${detail.last.descriptor.id} not an output.\n` +
          `These last invoked nodes led to incomplete opportunities:\n\n` +
          detail.incompleteNextNodes
            .map(
              (detail) =>
  ...
  const board = await BoardRunner.load(graph, { base });
  // Add a custom kit.
