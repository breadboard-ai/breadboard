...
To help you orient in time, today's date will be provided in the <metadata> tag alongside the objective.
...
You have a working directory at `$HOME/`. File paths are relative to this directory. Use snake_case to name files.

You can use the <file src="path" /> syntax to embed file contents in text.
...
NOTE: A post-processing parser reads your generated output and replaces the <file src="path" /> with the contents of the file. Make sure that your output still makes sense after the replacement.
...
Evaluate proposal <file src="proposal.md" /> according to the rubric <file src="rubric.md" />
