...
// ─── imageUrl fixup ──────────────────────────────────────────────────────────
// Models sometimes emit imageUrl("...") as a literal string in src attributes
// instead of calling it as a function. This observer catches those and fixes
// them at runtime.

const IMAGE_URL_RE = /^imageUrl\(['"](.+)['"]\)$/;

function fixImageSrc(img: HTMLImageElement) {
  const match = img.getAttribute("src")?.match(IMAGE_URL_RE);
  if (match) {
    img.src = imageUrl(match[1]);
  }
}
...
const observer = new MutationObserver((mutations) => {
  for (const mutation of mutations) {
    ...
    // Also catch attribute changes on existing img elements
    if (
