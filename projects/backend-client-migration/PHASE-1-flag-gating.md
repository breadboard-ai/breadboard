...
## Phase 7 — theme-utils.ts: Gemini image generation

**Depends on:** Phase 3.0 (A2ModuleArgs plumbing — already complete) — but this
file is NOT an A2 module. It's an SCA action utility that accesses
`fetchWithCreds` via the `AppServices` interface, which does not expose
`backendClient`.

This endpoint was missed during the original migration. It calls
`models/${model}:generateContent` (the same Gemini proxy endpoint migrated in
Phase 6b for `gemini.ts`) but through a completely different code path.

### Why it was missed

The Phase 1 migration covered files where `fetchWithCreds` was received as a
**constructor parameter** alongside `backendClient`. The theme utility is
different: it's a standalone function that receives the `AppServices` interface,
which exposes `fetchWithCreds` directly (line 62 of `services.ts`) but does
**not** expose `backendClient`. The `backendClient` is only injected into
individual service constructors during wiring, never surfaced on the
...
The `generateImage` function (line 39) calls:

```ts
const response = await services.fetchWithCreds(endpointURL(IMAGE_GENERATOR), {
  method: "POST",
...
where `endpointURL(model)` constructs
`` `${geminiApiPrefix()}/${encodeURIComponent(model)}:generateContent` ``.

### What to do

1. **Add `backendClient` to `AppServices`.** Add
   `backendClient: Promise<OpalBackendClient>` to the `AppServices` interface
   (in `services.ts`), and populate it from
   `config.shellHost.getOpalBackendClient()` in the `services()` factory. This
   aligns with the SCA pattern where services are the infrastructure layer, and
   makes `backendClient` available to any SCA action that receives `services` —
   preventing future code from falling into the same gap.

2. **Add the flag gate.** Follow the standard migration pattern:
   ...
   if (CLIENT_DEPLOYMENT_CONFIG.ENABLE_BACKEND_CLIENT) {
     ...
     response = await services.fetchWithCreds(endpointURL(IMAGE_GENERATOR), {
       method: "POST",
       body: JSON.stringify({ contents }),
       signal: abortSignal,
     });
   ...
   Note: body is passed as an object (not pre-stringified) in the
   `sendHttpRequest` path.

3. **Add tests for both paths** in
   [`packages/visual-editor/tests/sca/actions/theme/theme-utils.test.ts`](../../packages/visual-editor/tests/sca/actions/theme/theme-utils.test.ts).
   The existing test file already mocks `services.fetchWithCreds`. Add a
   `backendClientMock` with `sendHttpRequest` and test both flag states,
   following the pattern from `app-catalyst.test.ts`.

4. **Update `docs/dev/backend_reference.md`** — The
   `models/{model}:generateContent` entry already exists (for `gemini.ts`). Add
   `theme-utils.ts` as an additional call location.
...
Thorough coverage of `generateImage`: URL construction, error responses, empty
candidates, missing splash screen, status guards, inline/stored data variants,
and `persistTheme`. All tests mock `services.fetchWithCreds`. No
`ENABLE_BACKEND_CLIENT` tests.

Assessment: 🟢 Strong existing coverage. Adding flag-on tests is low effort —
duplicate the `fetchWithCreds` assertions to verify `sendHttpRequest` is called
...
| Endpoint                          | Trigger              | How to Verify                                                                                                   |
| --------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------- |
| `models/${model}:generateContent` | Explicit user action | Open an Opal → open Theme panel → click "Generate theme" with a prompt. Also triggered from FlowGen theme step. |

Verify: theme image is generated, palette is extracted, splash screen renders
correctly.

**Endpoints migrated after this phase:** 24 (cumulative — 23 from Phases 1–6
plus this one)
